...
inline bool isWater(landscapeCell *landscape, int mapSize, int x, int y, int prev, int &finish) {
...
inline int distanceToWater(landscapeCell *landscape, int mapSize, int start, int &finish, int step) {
	...
	while(1) {
		for (int i = x - dist; i <= x + dist; i++)
			if (isWater(landscape, mapSize, i, y - dist, (y - dist + 1)*mapSize + i, finish) || isWater(landscape, mapSize, i, y + dist, (y + dist - 1)*mapSize + i, finish))
				return dist;
		for (int i = y - dist + 1; i <= y + dist - 1; i++)
			if (isWater(landscape, mapSize, x - dist, i, i*mapSize + x - dist + 1, finish) || isWater(landscape, mapSize, x + dist, i, i*mapSize + x + dist - 1, finish))
				return dist;
